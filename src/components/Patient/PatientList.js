...
const PatientList = ({ refresh }) => {
  ...
  const fetchPatients = async () => {
    try {
      const response = await axios.get('https://backend-osa.vercel.app/patients');
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const response = await axios.put(`https://backend-osa.vercel.app/${id}/status`, { status });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://backend-osa.vercel.app/patients/${id}`);
