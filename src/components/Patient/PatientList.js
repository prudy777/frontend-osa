...
const PatientList = ({ refresh }) => {
  ...
  const fetchPatients = async () => {
    try {
      const response = await axios.get('https://frontend-osa.vercel.app/patients');
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const response = await axios.put(`https://frontend-osa.vercel.app/${id}/status`, { status });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://frontend-osa.vercel.app/patients/${id}`);
